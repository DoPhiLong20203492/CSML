...
def settings(param=[]):
    ...
    parser.add_argument('--run_num', type=int, default=6, help="run number")
    ...
    for epoch in range(params.epoch_num):
        ...
        if epoch % params.evaluate_step == 0:
            ...
            if best_info_train['Recall@1'] < train_acc_l[0]:
                ...
                best_info_train['model_params'] = model.state_dict() #chi can luu learning rate, optimizer, loss, precision recall
...
if __name__ == '__main__':
    ...
    # file name to store
    FILE_NAME = [params.path_out, f'{time.strftime("%Y%m%d")}_{params.data_name}']
    FILE_NAME[1] += f'{params.out_filename}'
    
    # Load data
    print(end = " ")
    print('='*21, ' Loading data')
    ...
    print(f'\nFinished, time cost is {time.time()-start_time:.1f}')
    ...
    print(f'Result saved. File name is {FILE_NAME[0]}model_{FILE_NAME[1]}.pkl')
