...
class Model(nn.Module):
    def __init__(self, params):
        ...
        # Capturer
        # Version: Seperate
        self.capturer_t = SessionCapturer(RNN_input_dim, self.rnn_t_hid_dim, params) #time's RNN
        self.capturer_l = SessionCapturer(RNN_input_dim, self.rnn_l_hid_dim, params) #location's RNN
        
        if self.cat_contained:
            self.cat_embedder = nn.Embedding(self.cid_size, self.cat_embed_dim, padding_idx=0)
            self.capturer_c = SessionCapturer(RNN_input_dim, self.rnn_c_hid_dim, params) #category's RNN
